                    ...
                    for (int i = messageCount; i > 0; i--)
                    {
                        ...
                        try
                        {
                            ...
                            foreach (var attachment in message.FindAllAttachments())
                            {
                                ...
                                connection.Execute("insert into Attachments (Id, Filename, ContentType, Contents, OrderId, UserId, Category, MessageId) values (@id, @fileName, @contentType, @contents, @orderId, @userId, 'Email Attachment', @messageId)"
                                    , new { id = Guid.NewGuid(), fileName = attachment.FileName,  contentType = contentType, contents = attachment.Body, orderId = orderId.Value, userId = userId.ToString(), messageId = messageId});
