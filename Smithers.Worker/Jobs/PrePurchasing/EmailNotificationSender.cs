...
namespace Smithers.Worker.Jobs.PrePurchasing
{
    public class EmailNotificationSender : Job<EmailNotificationSender>
    {
        ...
        private void ProcessEmails(EmailPreferences.NotificationTypes notificationType)
        {
            ...
            using (var connection = new ReliableSqlConnection(_connectionString).Open())
            {
                ...
                users = connection.Query("select distinct Id, Email from users where id in @ids",
        ...
        private void BatchEmail(string email, List<dynamic> pendingForUser)
        {
            ...
            //Do batches inside of their own connection and transaction
            using (var connection = new ReliableSqlConnection(_connectionString).Open())
            {
                using (var ts = connection.BeginTransaction())
                {
                    ...
                    foreach (var order in pendingOrders)
                    {
                        ...
                        message.Append(
                            string.Format("<tr><td style=\"width: 100px;\">Order Request</td><td>{0}</td></tr>",
                        ...
                        message.Append(
                            string.Format(
                                "<tr><td style=\"width: 100px;\"><strong>Vendor:</strong></td><td>{0}</td></tr>",
                        ...
                        foreach (
                            var emailQueue in
                                pendingForUser.Where(a => a.OrderId == orderid)
                                              .OrderByDescending(b => b.DateTimeCreated))
                        {
                            message.Append("<tr>");
                            message.Append(
                                string.Format(
                                    "<td style=\"padding-left: 7px; border-left-width: 0px; margin-left: 0px; width: 180px;\">{0}</td>",
                                    emailQueue.DateTimeCreated));
                            ...
                            connection.Execute(
                                "update EmailQueueV2 set Pending = 0, DateTimeSent = @now where id = @id",
                    ...
                    message.Append(string.Format("<p><em>{0} </em><em><a href=\"{1}\">{2}</a>&nbsp;</em></p>",
                                                 "You can change your email preferences at any time by",
                                                 "http://prepurchasing.ucdavis.edu/User/Profile",
                                                 "updating your profile on the PrePurchasing site"));
